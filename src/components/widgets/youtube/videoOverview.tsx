...
const YoutubeVideoWidgetOverview = (props: Props) => {
  const {
    id,
    title,
    video_id,
  ...
  const splitUrl = link?.split('https://www.youtube.com/watch?v=')[1];
  const urlId = splitUrl?.split('&')[0];
  // console.log(urlId);
  // console.log("SPLIT YOUTUBE URL => ", splitUrl);
  ...
  const style_var: object = styles ? styles : {}; // specifies any styles added to the widget
  ...
  const video_id_var: string = urlId ? urlId : null; // id of youtube video to create widget
  ...
  const controls_var: string = controls ? controls : '1'; // This parameter indicates whether the video player controls are displayed: controls=0 – Player controls do not display in the player. controls=1 (default) – Player controls display in the player.
  ...
  return (
    <div className='my-video-element roundBorder h-[100px] w-full  rounded-xl'>
      <iframe
...
export default YoutubeVideoWidgetOverview;
